...
class BookingController extends Controller
{
    ...
    public function startBooking(Booking $booking, Resource  $resource){
        $this->user = Auth::user();
        if(!$this->user) {
        ...
        try{
            $this->log[] = 'Booking start: ' . $booking->club->name . ' ' . $resource->name.' '.$booking->started_at->format('d-m-Y') . ' ' . $booking->timetable->name;
            $prebooking = $this->booking($booking,  $resource);
            ...
            $booking->name = $booking->club->name . ' ' . $resource->name.' '.$booking->started_at->format('d-m-Y') . ' ' . $booking->timetable->name;
    ...
    public function booking(Booking $booking, Resource  $resource){
        $this->log[] = 'Prebooking '.$booking->timetable->name;
        $prebooking = $this->getPreBooking($booking, $resource, $booking->timetable);
        if(isset($prebooking['status']) && $prebooking['status'] === 'fail') return ['error' => 'Prebooking error '.$prebooking['message']];
        $this->log[] = 'Payment method '.$booking->timetable->name;
        ...
        $this->log[] = 'Confirmation '.$booking->timetable->name;
        ...
        $this->log[] = 'Confirmation Match '.$booking->timetable->name;
        ...
        $this->log[] = '  Booking end: ' . $booking->name . ' ' . $resource->name.' '.$booking->started_at->format('d-m-Y') . ' ' . $booking->timetable->name . ' Do it!';
        if (!isset($prebooking['error'])) {
            Mail::to($this->user)->send(new PlaytomicBookingConfirmation($booking, $resource, $booking->timetable, $prebooking));
