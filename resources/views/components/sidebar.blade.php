                ...
                <ul class="space-y-2">
                    ...
                    <li>
                        ...
                        <ul id="dropdown-playtomic" class="@if(Route::is('playtomic.*')) '' @else hidden @endif py-2 space-y-2">
                            ...
                            @endcan
                            @can('playtomic.resources_access')
                    ...
                    <li>
                        <button type="button" class="flex items-center w-full font-medium text-sm hover:text-black transition ease-in-out duration-500 hover:bg-gray-100 dark:hover:bg-gray-700 p-2 rounded-lg focus:outline-none"
                                aria-controls="dropdown-lineups" data-collapse-toggle="dropdown-lineups">
                            <i class="fad fa-dot-circle text-xs"></i>
                            <span class="flex-1 ml-3 text-left whitespace-nowrap uppercase" sidebar-toggle-item>{{ trans('line-ups.title') }}</span>
                            <svg sidebar-toggle-item class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd">
                                </path>
                            </svg>
                        </button>
                        <ul id="dropdown-lineups" class="@if(Route::is('line-ups.*')) '' @else hidden @endif py-2 space-y-2">
                            @can('line-ups.dashboard')
                                <li class="hover:text-black hover:bg-teal-300 pt-2 pt-1 rounded-lg items-center">
                                <a href="{{ route("line-ups.dashboard.index") }}" class="flex mb-3 pl-4 capitalize font-medium text-sm hover:text-black transition ease-in-out duration-500
                                @if(Route::is('line-ups.dashboard.index')) text-warning-600 @endif">
                                    <i class="fad fa-clock text-xs mr-2"></i>
                                    {{ trans('line-ups.dashboard.title') }}
                                </a>
                                </li>
                            @endcan
                            @can('line-ups.administration_access')
                                <li class="hover:text-black hover:bg-teal-300 pt-2 pt-1 rounded-lg items-center">
                                    <a href="{{ route("line-ups.administration.index") }}" class="flex mb-3 pl-4 capitalize font-medium text-sm hover:text-black transition ease-in-out duration-500
                                @if(Route::is('line-ups.administration.index')) text-warning-600 @endif">
                                        <i class="fad fa-clock text-xs mr-2"></i>
                                        {{ trans('line-ups.administration.title') }}
                                    </a>
                                </li>
                            @endcan
                            @can('line-ups.calendar_index')
                                    <li class="hover:text-black hover:bg-teal-300 pt-2 pt-1 rounded-lg items-center">
                                <a href="{{ route("line-ups.dashboard.calendar.index") }}" class="flex mb-3 pl-4 capitalize font-medium text-sm hover:text-black transition ease-in-out duration-500
                                @if(Route::is('line-ups.dashboard.calendar.*')) text-warning-600 @endif">
                                    <i class="fad fa-calendar text-xs mr-2"></i>
                                    {{ trans('line-ups.calendar.title') }}
                                </a>
                                </li>
                            @endcan
                            @can('line-ups.round_access')
                                    <li class="hover:text-black hover:bg-teal-300 pt-2 pt-1 rounded-lg items-center">
                                <a href="{{ route("line-ups.administration.round.index") }}" class="flex mb-3 pl-4 capitalize font-medium text-sm hover:text-black transition ease-in-out duration-500
                                @if(Route::is('line-ups.administration.round.*')) text-warning-600 @endif">
                                    <i class="fad fa-clock text-xs mr-2"></i>
