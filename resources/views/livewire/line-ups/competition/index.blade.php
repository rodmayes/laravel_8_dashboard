<div class="card">
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <div class="flex items-center justify-between bg-white dark:bg-gray-900">
            <div class="flex">
                ...
                <div class="flex-initial px-4 py-2 m-2">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white required"
                           for="years">{{ trans('line-ups.competitions.per_year') }}:</label>
                    <x-select
                        placeholder="{{ trans('line-ups.competitions.select_year') }}"
                ...
                <div class="flex-initial px-4 py-2 m-2">
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white required"
                           for="teams">{{ trans('line-ups.competitions.per_team') }}:</label>
                    <x-select
                        placeholder="{{ trans('line-ups.competitions.select_team') }}"
            ...
            <div class="flex flex-row-reverse px-4 py-2 m-2">
                ...
                <button type="button" wire:click="$emit('openModal','line-ups.competition.create')"
                        class="btn bg-indigo-700 hover:bg-white text-white hover:text-indigo-700 py-2 px-4 mr-2 border rounded">
                    <i class="fa fa-plus-circle"></i> {{ trans('global.create') }} {{ trans('line-ups.competitions.title') }}
                </button>
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tr>
                ...
                <th scope="col" class="px-6 py-3">
                    {{ trans('line-ups.competitions.fields.id') }}
                    @include('components.table.sort', ['field' => 'id'])
                </th>
                <th scope="col" class="px-6 py-3">
                    {{ trans('line-ups.competitions.fields.name') }}
                    @include('components.table.sort', ['field' => 'name'])
                </th>
                <th scope="col" class="px-6 py-3">
                    {{ trans('line-ups.competitions.fields.couples_number') }}
                </th>
                <th scope="col" class="px-6 py-3">
                    {{ trans('line-ups.years.title') }}
                </th>
            ...
            @forelse($competitions as $competition)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-2 py-2">{{ $competition->id }}</td>
                    <td class="px-2 py-2">{{ $competition->name }}</td>
                    <td class="px-2 py-2">{{ $competition->couples_number }}</td>
                    <td class="px-2 py-2">{{ $competition->year->name }}</td>
                    <td class="px-2 py-2">
                        <div class="flex justify-end">
                            @can('line-ups.competition_show')
                                <button data-modal-target="competitionShowModal"
                                        data-modal-toggle="competitionShowModal"
                                        wire:click="showItem({{ $competition }})" wire:loading.attr="disabled"
                            ...
                            @endcan
                            @can('line-ups.competition_edit')
                                    <button type="button" wire:click="$emit('openModal','line-ups.competition.edit', {{ json_encode([$competition->id]) }})"
                            ...
                            @endcan
                            @can('line-ups.competition_delete')
                                <button
                                    class="px-2.5 py-2 text-xs text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg mr-1 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800"
                                    wire:click="confirmDelete({{ $competition->id }})" wire:loading.attr="disabled"
        ...
        <nav class="flex items-center justify-between p-4 " aria-label="Table navigation">
            <span class="text-sm font-normal text-gray-500 dark:text-gray-400"> <span
                    class="font-semibold text-gray-900 dark:text-white">{{$competitions->currentPage()}}</span> of <span
                    class="font-semibold text-gray-900 dark:text-white">{{ $competitions->total() }}</span></span>
            ...
            <ul class="inline-flex -space-x-px text-sm h-8">
                <li>
                    <a href="{{$competitions->previousPageUrl()}}"
                ...
                @foreach($competitions->getUrlRange(1,ceil($competitions->total()/$competitions->perPage())) as $index => $page)
                    <li>
                        <a href="{{$competitions->url($index)}}"
                ...
                @endforeach
                <li>
                    <a href="{{$competitions->nextPageUrl()}}"
    ...
    @include('livewire.line-ups.competition.modal.show')
</div>
