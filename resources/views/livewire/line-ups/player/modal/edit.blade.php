...
<div class="bg-white p-2 sm:px-6 sm:py-4 border-b border-gray-150">
    ...
    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
            {{ trans('global.edit') }} {{ trans('line-ups.players.title') }}
        </h3>
    ...
    <form wire:submit.prevent="submit" class="pt-3">
        ...
        <div class="{{ $errors->has('player.position') ? 'invalid' : '' }}">
            <label class="form-label required" for="position">{{ trans('line-ups.players.fields.position') }}</label>
            <x-select
                placeholder="Select position"
                :options="$listsForFields['positions']"
        ...
        <div class="{{ $errors->has('competitions') ? 'invalid' : '' }}">
            <label class="form-label required" for="competition">{{ trans('line-ups.players.fields.competition') }}</label>
            <x-select
                placeholder="Select competitions"
                :options="$listsForFields['competitions']"
                wire:model.defer="competitions"
                option-value="id"
                option-label="fullname"
            />
            ...
            <div class="help-block">
                {{ trans('line-ups.players.fields.competition_helper') }}
            </div>
        </div>
        <div class="w-full mb-2">
