    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <div class="flex items-center justify-between bg-white dark:bg-gray-900">
            ...
            <div class="flex flex-row-reverse px-4 py-2 m-2">
                ...
                <button type="button" wire:click="$emit('openModal','line-ups.year.create')"
                        class="btn bg-indigo-700 hover:bg-white text-white hover:text-indigo-700 py-2 px-4 mr-2 border rounded">
                    <i class="fa fa-plus-circle"></i> {{ trans('global.create') }} {{ trans('line-ups.years.title') }}
                </button>
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tr>
                ...
                <th scope="col" class="px-6 py-3">
                    {{ trans('line-ups.years.fields.id') }}
                    @include('components.table.sort', ['field' => 'id'])
                </th>
                <th scope="col" class="px-6 py-3">
                    {{ trans('line-ups.years.fields.name') }}
        ...
        <tbody class="text-left text-gray-600">
            @forelse($years as $year)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td class="w-4 p-4">
                        <div class="flex items-center">
                            <input type="checkbox" value="{{ $year->id }}" wire:model="selected" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        </div>
                    </td>
                    <td class="px-2 py-2">{{ $year->id }}</td>
                    <td class="px-2 py-2">{{ $year->name }}</td>
                    <td class="px-2 py-2">
                        <div class="flex justify-end">
                            @can('line-ups.year_show')
                                <button data-modal-target="year-show-modal" data-modal-toggle="year-show-modal" wire:click="showItem({{ $year}})" wire:loading.attr="disabled"
                            ...
                            @can('line-ups.year_edit')
                                <button type="button" wire:click="$emit('openModal','line-ups.year.edit', {{ json_encode([$year->id]) }})"
                            ...
                            @can('line-ups.year_delete')
                                <button wire:click="confirmDelete({{ $year->id }})" wire:loading.attr="disabled" title="{{ trans('global.delete') }}"
        ...
        <nav class="flex items-center justify-between p-4 " aria-label="Table navigation">
            <span class="text-sm font-normal text-gray-500 dark:text-gray-400"> <span class="font-semibold text-gray-900 dark:text-white">{{$years->currentPage()}}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ $years->total() }}</span></span>
            ...
            <ul class="inline-flex -space-x-px text-sm h-8">
                <li>
                    <a href="{{$years->previousPageUrl()}}" class="flex items-center justify-center px-3 h-8 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</a>
                </li>
                @foreach($years->getUrlRange(1,ceil($years->total()/$years->perPage())) as $index => $page)
                    <li>
                        <a href="{{$years->url($index)}}" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">{{$index}}</a>
                    </li>
                @endforeach
                <li>
                    <a href="{{$years->nextPageUrl()}}" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-r-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</a>
                </li>
            </ul>
        </nav>

        @include('livewire.line-ups.year.modal.show')
